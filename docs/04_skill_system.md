...
**新脚本应使用 `ctx.*` 访问上下文变量**（推荐写法）：

```javascript
// 推荐：使用 ctx 对象访问上下文
let user_input = ctx.user_input;           // 用户输入
let symbol = ctx.symbol ?? "600519";       // 自定义上下文变量
let threshold = ctx.threshold ?? 3.0;
```

**兼容性说明**：为保持向后兼容，以下历史写法仍然可用（但不推荐新脚本使用）：

```javascript
// 兼容写法 1：使用 context 对象（历史脚本）
let user_input = context["user_input"];
let symbol = context["symbol"];

// 兼容写法 2：直接访问顶层变量（历史脚本）
let input = user_input;
let sym = symbol;
```

三种访问方式在当前版本均可正常工作，但 `ctx.*` 是规范写法，具有更好的可维护性和一致性。

### 迁移指南

**当前阶段**：系统同时支持三种访问方式（`ctx.*`、`context[...]`、顶层变量），确保历史脚本无需修改即可运行。

**推荐做法**：
- 新编写的脚本应统一使用 `ctx.*` 访问上下文变量
- 历史脚本可以继续使用原有写法，系统会保持兼容
- 在方便时可以逐步将历史脚本迁移到 `ctx.*` 写法
